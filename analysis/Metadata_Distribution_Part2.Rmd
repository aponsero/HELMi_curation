...
```{r, echo=FALSE}
curr_Voc <- Voc %>% filter(Questionnaire=="E_Previous3Months")

# Looking at boolean fields
bool_fields <- curr_Voc %>% filter(data_type=="bool") %>% pull(variable_short_name)
curr_bool <- Q2 %>% select(familly_ID, all_of(bool_fields))
...
curr_cat <- Q2 %>% select(familly_ID, all_of(cat_fields)) %>% mutate_all(as.character)
...
```{r,  warning=FALSE, echo=FALSE, message=FALSE, fig.height=15}
curr_Voc <- Voc %>% filter(Questionnaire=="E_Previous3Months")
...
curr_int <- Q2 %>% select(all_of(int_fields)) %>% 
...
*** Empty boolean fields in this questionnaire***
```{r, echo=FALSE}
curr_Voc <- Voc %>% filter(Questionnaire=="C_4to6months")

# Looking at boolean fields
bool_fields <- curr_Voc %>% filter(data_type=="bool") %>% pull(variable_short_name)
curr_bool <- Q3 %>% select(familly_ID, all_of(bool_fields))
curr_bool_long <- curr_bool %>% pivot_longer(!familly_ID, names_to="question", values_to="answers")
curr_bool_counts <- curr_bool_long %>% group_by(question, answers) %>% tally()
...
cat_fields <- curr_Voc %>% filter(data_type=="categorical") %>%
  pull(variable_short_name)
curr_cat <- Q3 %>% select(familly_ID, all_of(cat_fields)) %>% mutate_all(as.character)
curr_cat_long <- curr_cat %>% pivot_longer(!familly_ID, names_to="question", values_to="answers") 
curr_cat_counts <- curr_cat_long %>% group_by(question, answers) %>% tally() 
...
curr_int <- Q3 %>% select(all_of(int_fields)) %>% 
  mutate_if(is.character,as.numeric)
...
```{r, echo=FALSE}
curr_Voc <- Voc %>% filter(Questionnaire=="D_7to12months")

# Looking at boolean fields
bool_fields <- curr_Voc %>% filter(data_type=="bool") %>% pull(variable_short_name)
curr_bool <- Q4 %>% select(familly_ID, all_of(bool_fields))
curr_bool_long <- curr_bool %>% pivot_longer(!familly_ID, names_to="question", values_to="answers")
curr_bool_counts <- curr_bool_long %>% group_by(question, answers) %>% tally()
...
cat_fields <- curr_Voc %>% filter(data_type=="categorical") %>%
  pull(variable_short_name)
curr_cat <- Q4 %>% select(familly_ID, all_of(cat_fields)) %>% mutate_all(as.character)
curr_cat_long <- curr_cat %>% pivot_longer(!familly_ID, names_to="question", values_to="answers") 
curr_cat_counts <- curr_cat_long %>% group_by(question, answers) %>% tally() 
...
curr_int <- Q4 %>% select(all_of(int_fields)) %>% 
  mutate_if(is.character,as.numeric)
...
```{r, echo=FALSE}
curr_Voc <- Voc %>% filter(Questionnaire=="B_0to3months")

# Looking at boolean fields
bool_fields <- curr_Voc %>% filter(data_type=="bool") %>% pull(variable_short_name)
curr_bool <- Q9 %>% select(familly_ID, all_of(bool_fields))
curr_bool_long <- curr_bool %>% pivot_longer(!familly_ID, names_to="question", values_to="answers")
curr_bool_counts <- curr_bool_long %>% group_by(question, answers) %>% tally()
...
cat_fields <- curr_Voc %>% filter(data_type=="categorical") %>%
  pull(variable_short_name)
curr_cat <- Q9 %>% select(familly_ID, all_of(cat_fields)) %>% mutate_all(as.character)
curr_cat_long <- curr_cat %>% pivot_longer(!familly_ID, names_to="question", values_to="answers") 
curr_cat_counts <- curr_cat_long %>% group_by(question, answers) %>% tally() 
...
curr_int <- Q9 %>% select(all_of(int_fields)) %>% 
  mutate_if(is.character,as.numeric)
